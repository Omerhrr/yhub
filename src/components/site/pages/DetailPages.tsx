...
export function EventsListPage() {
  ...
  React.useEffect(() => { setMonthFilter("all"); }, [tab]);

  // Unique months from past events, newest first
  const pastMonths = React.useMemo(() => {
    const seen = new Set<string>();
    past.forEach(e => { if (e.date) seen.add(e.date.slice(0, 7)); });
  ...
  const filteredPast = monthFilter === "all" ? past : past.filter(e => e.date?.startsWith(monthFilter));
  const displayed = tab === "active" ? active : filteredPast;
  ...
  return (
    <div className="container mx-auto max-w-6xl px-4 py-12">
      ...
      {/* Month filter — only shown on past tab */}
      {tab === "past" && pastMonths.length > 0 && (
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
        {displayed.length === 0 ? (
          <p className="col-span-full text-center text-muted-foreground py-12">No events found for this period.</p>
        ) : displayed.map((e) => (
          <EventListCard
            key={e.id}
            event={e}
